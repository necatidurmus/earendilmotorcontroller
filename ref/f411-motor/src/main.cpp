...
// Hall ham değeri geçersiz olursa son geçerli state tutulur.
// 50ms — hall sensör gürültüsü/switching transient için tolerans.
// Eskiden 15ms'di, bu gürültülü hall'de coast'a (boşa dönme) yol açıyordu.
static constexpr uint32_t INVALID_HALL_HOLD_US       = 50000;
// Hall geçersiz kalma süresi — sonrasında motor durdurulur
// 100ms — gerçekten kalıcı hall hatası eşiği
static constexpr uint32_t INVALID_HALL_STOP_US       = 100000;

// Geçersiz hall geçiş sayısı fault eşiği
// 50 — gürültülü hall ortamında 20 çok agresifti, yüksek RPM'de false fault.
// Her 100 geçerli geçişte sıfırlanır (updateHallRuntime).
static constexpr uint8_t  INVALID_TRANSITION_THRESHOLD = 50;
...
static constexpr uint32_t RPM_FEEDBACK_TIMEOUT_MS     = 5000;
...
struct PidRuntime {
  ...
  // Speed mode fault retry
  uint8_t faultRetryCount = 0;
};
...
void resetPidState() {
  ...
  // NOT: faultRetryCount burada sıfırlanmaz — sadece SpeedPI'da sıfırlanır
}
...
uint32_t calculateRPM() {
  ...
  // Son hall geçişi 150ms'den eski ise RPM = 0 (stale guard)
  // Düşük hızda PI loop'un gerçek durumu görmesi için kısaltıldı
  uint32_t sinceLastTransition = micros() - hallRt.prevTransitionUs;
  if (sinceLastTransition > 150000UL) return 0;
...
void runSpeedControlLoop(uint32_t nowMs) {
  ...
  if (!isMotorDriveActive() && motorRt.phase != MotorPhase::NeutralWait) {
    // Speed mode fault recovery — Fault'taysa Stopped'a çevir, dene
    if (motorRt.phase == MotorPhase::Fault) {
      if (pidRt.faultRetryCount >= 3) {
        disablePidMode();
        CMD.println(F("[WARN] Speed fault limit — PI OFF"));
        return;
      }
      pidRt.faultRetryCount++;
  ...
  if (pidRt.lastHallEventMs != 0 && (uint32_t)(nowMs - pidRt.lastHallEventMs) > RPM_FEEDBACK_TIMEOUT_MS) {
    ...
    // NOT: disablePidMode() kaldırıldı — PI loop motoru otomatik restart eder
    return;
  }
  ...
  // Speed mode'da PI aktifken watchdog'u ve host connection monitor'u yenile.
  // PI loop host'dan periyodik komut beklemeden motoru sürdüğü için
  // checkCommandWatchdog/checkHostConnection timeout'a düşmemeli.
  if (isMotorDriveActive()) {
    lastMotorCommandMs = nowMs;
    lastUartActivityMs = nowMs;
  }
}
...
void updateHallRuntime(uint32_t mainUs) {
  ...
  // [bugfix] Geçerli durum ama geçersiz geçiş (atlamalı) — motor yine de
  // geçerli bir durum izliyor, lastValidUs taze tutulmalı ki startup timeout
  // (START_NO_HALL_TIMEOUT_MS) false fault vermesin.
  ...
  // Periyodik sıfırlama — saatlerce çalışırken false fault önleme
  if (hallRt.validTransitionCount % 100 == 0) {
    hallRt.invalidTransitionCount = 0;
  }
}
...
bool parseLongAfterPrefix(const char* s, const char* prefix, long* out) {
  ...
  if (errno == ERANGE) {
    *out = (v == LONG_MIN) ? LONG_MIN : LONG_MAX;
    return true;
  }
  *out = v;
  return true;
...
void processCommand(char* cmd, CommandSource src) {
  ...
  if (parseLongAfterPrefix(cmd, "rpm ", &v)) {
    // rpm 0: speed moduna bakmaksızın durdur (H7 stopAll hem rpm 0 hem stop gönderir)
    if (v == 0) {
      if (pidRt.enabled) {
        pidRt.targetRpmCmd = 0;
      }
    ...
    // rpm <val> (val != 0): speed mode değilse otomatik aç.
    // H7 "rpm forward 50" → F411 "rpm 50" gönderir; kullanıcı manuel
    // "mode speed" demek zorunda kalmamalı.
    if (!pidRt.enabled) {
      pidRt.enabled = true;
      pidRt.controlMode = SpeedControlMode::Speed;
      resetPidState();
      sysLn(F("[INFO] Auto speed mode ON (rpm cmd)"));
    }

    pidRt.targetRpmCmd = (int32_t)v;
    // NOT: lastHallEventMs burada güncellenmez — PI loop ve hall ISR yönetir.
    // Heartbeat rpm komutu motor stall algılamasını maskelerdi.
    lastMotorCommandMs = millis();
    // Yön değişimi kontrolü
...
void motorControlTick(uint32_t nowUs) {
  ...
  if (motorRt.phase == MotorPhase::NeutralWait) {
    allOff();
    lastMotorCommandMs = millis(); // watchdog yenile — yön değişiminde stop önle
  ...
  if (isValidMappedState(stableState)) {
    ...
    // [bugfix] Geçerli durum sürülüyorsa invalid timer'ı sıfırla.
    // updateHallRuntime'da invalid transition (atlamalı geçiş) invalidSinceUs'i
    // set eder ama motor geçerli durumu sürer. Bu süre sonradan 000/111 gelince
    // yanılgılı şekilde hold timer'ı tüketirdi — coast'a (boşa dönme) yol açardı.
...
// Kompakt format: RPM:<measured>,T:<target>,D:<duty>,DIR:<F/R>,PH:<phase>,SP:<0/1>,BRAKE:<0/1>,FC:<code>,H:<hall>,PWM_SET:<targetDuty>,PWM_ACT:<currentDuty>
// Debug format:   RPM:<measured>,RF:<filtered>,Tcmd:<target>,Trmp:<ramped>,ERR:<error>,BASE:<base>,OUT:<pi_output>,I:<integral>,D:<duty>,PH:<speed_phase>,FC:<code>,PWM_SET:<targetDuty>,PWM_ACT:<currentDuty>
void sendTelemetry(uint32_t nowMs) {
  ...
  if (telemetryDebug && pidRt.enabled) {
    ...
    // OUT: actual PI output (basePwm + P + I) — pidRt.piOutput runSpeedControlLoop'da saklanir
    // BASE: feed-forward base PWM (statik sürtünme telafisi)
    float error = pidRt.targetRpmRamped - pidRt.filteredRpm;
    ...
    CMD.print(F(",BASE:"));
    CMD.print((int32_t)pidRt.basePwm);
    CMD.print(F(",OUT:"));
    CMD.print((int32_t)pidRt.piOutput);
    CMD.print(F(",I:"));
    ...
    CMD.print(F(",PWM_ACT:"));
    CMD.println(motorRt.currentDuty);
    return;
  }
...
// Speed mode: PI loop motoru otomatik sürdüğü için host disconnect kontrolü
// bypass edilir (lastUartActivityMs runSpeedControlLoop içinde yenilenir).
...
void checkHostConnection(uint32_t nowMs) {
  ...
  if (pidRt.enabled) return;      // speed mode: PI host'dan bağımsız sürer
