...
/* D4 fix: safe absolute value that avoids undefined behavior for
 * INT32_MIN.  Standard abs(INT32_MIN) is UB in C; casting to
 * unsigned first and then negating is well-defined because unsigned
 * arithmetic wraps. */
...
void Telemetry_Tick(uint32_t nowMs)
{
    ...
    if (s_mode == TELEMETRY_DEBUG) {
        ...
        uint32_t target = App_IsSpeedMode()
                          ? (uint32_t)SAFE_ABS(raw_target) : 0U;
        uint8_t  setd   = App_GetTargetDuty();
        uint8_t  actd   = MotorDriver_GetDuty();
        FaultCode fc    = FaultManager_GetLast();
