...
void MotionControl_Service(void)
{
    ...
    /* Do not clobber gate test / service task outputs. */
    if (s->gatetest_active) return;
    if (ServiceTask_IsActive()) return;

    /* SpeedPI internal fault. */
    ...
    uint8_t apply_duty = s->current_duty;
    if (SpeedPI_IsEnabled()) {
