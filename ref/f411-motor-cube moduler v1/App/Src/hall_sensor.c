...
void HallSensor_Update(void)
{
    ...
    if (raw == s_hall.stableRaw) {
        ...
        if (!Commutation_IsValidState(newState)) {
            ...
            /* P1-001: immediately invalidate lastValidState so
             * service_motor() detects it and calls AllOff without
             * waiting for the 100ms persistence timer.  Previously
             * the motor kept driving the old sector for up to 100ms
             * when Hall sensors gave an invalid code. */
            s_hall.lastValidState = HALL_STATE_INVALID;
        } else {
...
{
    return Commutation_IsValidState(Commutation_HallToState(s_hall.stableRaw));
}
...
{
    /* Re-evaluate the current raw Hall code against the new map. */
    uint8_t raw = read_raw_hall();
    uint8_t mapped = Commutation_HallToState(raw);
    ...
    s_hall.rawCandidate = raw;
    s_hall.rawCandidateCount = 1U;
    s_hall.candidateStartUs = App_GetMicros();

    if (Commutation_IsValidState(mapped)) {
        s_hall.lastValidState    = mapped;
        s_hall.lastDrivenState   = mapped;
        s_hall.invalidSinceUs    = 0U;
    } else {
        s_hall.lastValidState    = HALL_STATE_INVALID;
        s_hall.lastDrivenState   = HALL_STATE_INVALID;
        s_hall.invalidSinceUs    = App_GetMicros();
    }

    /* Reset edge/RPM tracking so stale data from the old map
     * does not affect the new map evaluation. */
    s_hall.prevTransitionUs    = 0U;
    s_hall.hallPeriodUs        = 0U;
    s_hall.lastTransitionUs    = 0U;
    s_hall.lastValidUs         = App_GetMicros();

    /* Reset counters */
    s_hall.invalidRawCount         = 0U;
    s_hall.invalidTransitionCount  = 0U;
    s_hall.validTransitionCount    = 0U;
    s_edge_counter                 = 0U;

    /* Clear any Hall fault */
    s_hall.fault = HALL_FAULT_NONE;
