...
/* ── Line assembler buffer size ──────────────────────────────────────────────
 *  Must accommodate the longest single line from F411 (status block ~100
 *  chars).  A DMA chunk may contain multiple lines; the assembler
 *  accumulates partial lines across chunks and emits on '\n'. */
#define MOTOR_LINE_BUF_SIZE  192
...
/* ── Line assembler: accumulates bytes across DMA chunks, emits on \n ──── */
typedef struct {
...
void MotorUartDma_Init(void)
{
    ...
    memset(lineAsm, 0, sizeof(lineAsm));
...
void MotorUartDma_Update(void)
{
    ...
    for (int i = 0; i < NUM_MOTOR_UARTS; i++)
    {
        if (rxSlot[i].ready)
        {
            ...
            /* Detect compact F411 telemetry (contains RPM:, PWM_ACT:).
             * If matched, re-log with [TEL][MOTOR] tag so the GUI can parse
             * it unambiguously.  This log respects the telemetry bridge
             * state — when bridge is OFF, telemetry forwarding is
             * suppressed, but all RX processing (SafetyManager_NotifyRx,
             * ACK parsing, link-loss detection) remains active. */
                ...
                strstr(line, "PWM_ACT:") != NULL)
            {
                ...
                /* else: telemetry suppressed, but RX processing continues */
            }
            else
            {
                /* Non-telemetry lines (ACK, ERROR, INFO, debug, etc.)
                 * are always forwarded regardless of bridge state. */
...
/* ── Line assembler: called from ISR context ──────────────────────────────
 *  Accumulates received bytes in a per-UART line buffer, emitting a
 *  complete line to rxSlot whenever a '\n' is encountered.  This handles:
 *  
 *  1. Multiple lines in a single DMA chunk (e.g. "[OK]\nRPM:60,...\n")
 *  2. A line split across two DMA chunks
 *  3. '\r\n' line endings (CR is skipped)
 *  4. Overflow: on buffer full, the current line is discarded; the next
 *     line after '\n' starts clean. */
static void AssembleLine(int idx, const uint8_t *data, uint16_t size)
{
    for (uint16_t i = 0; i < size; i++)
    {
        ...
        if (c == '\n')
        {
            /* End of line — emit if we have content and no overflow */
            ...
            /* Reset assembler for next line */
            lineAsm[idx].pos = 0;
            lineAsm[idx].overflow = false;
            continue;
        }

        /* Regular byte — accumulate if not in overflow */
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
    ...
    {
        /* Use line assembler to split received data into complete lines */
        AssembleLine(idx, diag[idx].dmaBuf, Size);
