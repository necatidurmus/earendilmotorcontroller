...
### Phase 0: Masa Başı Düzeltmeler (Ölçüm Gerektirmez)

**Öncelik sırası:**

| Sıra | Sorun | Kaynak | Tür | Açıklama |
|---|---|---|---|---|
| 0.1 | SysTick öncelik çakışması | KN#4 (CRITICAL) | Kod | TICK_INT_PRIORITY=0 → 15 yap |
| 0.2 | Volatile eksikliği | KN#5 (CRITICAL) | Kod | protection.c'deki ISR-shared değişkenlere volatile ekle |
| 0.3 | INT32_MIN UB | KN#6 (CRITICAL) | Kod | cli.c:79 val = -val overflow guard |
| 0.4 | nowUs overflow ~72 dk | KN#11 (MEDIUM) | Kod | main.c:78 uint32_t → 64-bit veya wrap-around |
| 0.5 | ADC EOC timeout yok | KN#9 (MEDIUM→HIGH) | Kod | board_io.c:352 while(EOC)'ye timeout sayacı ekle |
| 0.6 | cliPrintFloat overflow | KN#8 (MEDIUM) | Kod | cli.c buf[8] → buf[16] veya decimals clamp |
| 0.7 | EMA filtre init koşulu | KN#10 (MEDIUM) | Kod | protection.c'de bool filterInitialized flag ekle |
| 0.8 | gain help uyumsuz | KN#12 (MEDIUM) | Kod/cli | cli.c help mesajı ile kod aralığını eşitle |
| 0.9 | CCER direkt atama | KN#13 (MEDIUM) | Kod | bldc_commutation.c maskeli read-modify-write |
| 0.10 | FaultState dead code | KN#7 (HIGH) | Kod | protection.h FaultState sil veya kullan |
| 0.11 | Hall_SetDirection yorum | N1 (MEDIUM) | Kod | hall.h:62 yorumunu düzelt |
| 0.12 | Hall_GetDriveState ölü kod | N2 (LOW) | Kod | Sil veya "diagnostic only" olarak işaretle |
| 0.13 | cli.c gereksiz extern | N4 (LOW) | Kod | cli.c:48 extern huart2 satırını sil |
| 0.14 | platformio.ini build_unflags | KN#14 (MEDIUM) | Config | Anlamsız build_unflags satırını temizle |
| 0.15 | MOE runtime check | KN#26 (LOW) | Kod | board_io.c PWM init sonrası MOE assert ekle |
| 0.16 | Motor_config.h clock yorumları | KN#15,16 (HIGH) | Doc | 100 MHz → 96 MHz, Arduino referansları temizle |
| 0.17 | board_io.c ADC clock yorumları | KN#17 (HIGH) | Doc | PCLK2=96, ADC=24 MHz düzelt |
| 0.18 | board_io.h SYSCLK yorumu | KN#18 (LOW) | Doc | "100 MHz" → "96 MHz" |
| 0.19 | known_issues.md outdated bölüm 4 | KN#19 (LOW) | Doc | Arduino clock değerlerini güncelle |
| 0.20 | Prot_ClearFault null-term | KN#21 (LOW) | Kod | Explicit null-termination ekle |
| 0.21 | Hall pin speed | KN#20 (LOW) | Kod | GPIO_SPEED_FREQ_HIGH → LOW |
| 0.22 | RCC GPIOA tekrarlı | KN#23 (LOW) | Kod | Tekrarlı RCC enable'i temizle |
| 0.23 | Dead-time slew belgeleme | N3 (LOW) | Doc | control_strategy.md'ye slew açıklaması ekle |
...
|---|---|---|---|
| 2.1 | Synchronous complementary mi kalınacak? | Kullanıcı | Phase 1 osiloskop sonuçlarına göre |
| 2.2 | ADC DMA'ye geçiş | Kullanıcı | ISR blocking ADC sorunu çözüldükten sonra, timing gereksinimlerine göre |
| 2.3 | USB CDC'ye geçiş | Kullanıcı | Clock-tree doğru (zaten doğru), UART bring-up tamamlandıktan sonra |
...
| 3.4 | MOE runtime check | PWM init sonrası MOE biti doğrulama |
