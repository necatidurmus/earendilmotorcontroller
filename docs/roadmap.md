...
| C2 | Clock PLLQ uygun değil | **Sorun yok** — PLLQ=4, VCO=192 MHz. | Yok | Aksiyon gerekmiyor |
...
| C5 | Clock-tree uygun değil | **Yanlış değerlendirme** — clock-tree hedefe uygun. | Yok | Aksiyon gerekmiyor |
...
| 2.2 | ADC DMA'ye geçiş | Kullanıcı | ⏸️ Ertelendi (erken) |
| 2.3 | TIM1 Break / OCP | Kullanıcı | Donanımda OCP hattı netleşince |

### Phase 3: Güvenlik Eksiklikleri (Uzun Vade)

| Sıra | Konu | Açıklama |
|---|---|---|
| 3.1 | ~~IWDG watchdog~~ | ✅ Eklendi (IWDG init + main loop refresh) |
| 3.2 | ~~Undervoltage koruma~~ | ✅ Kodlandı (fault + strike + histerezis), bench kalibrasyon bekliyor |
