...
void MotorDriver_ActiveBrake(void)
{
     ...
     * This creates a magnetic braking effect by shorting all motor
     * windings through the low-side MOSFETs.
     *
     * Arduino equivalent:
     *   analogWrite(AH_PIN, 0);   // high-side OFF
     *   analogWrite(BH_PIN, 0);
     *   analogWrite(CH_PIN, 0);
     *   digitalWrite(AL_PIN, HIGH); // low-side ON
     *   digitalWrite(BL_PIN, HIGH);
     *   digitalWrite(CL_PIN, HIGH);
     *
     * Implementation: clear all CCxE (high-side enable), set all
     * CCxNE (low-side enable), force OCxM to inactive so OCxREF=0
     * and complementary output OCxN is HIGH.
     *
     * WARNING: No current sense — braking current cannot be measured.
     * Use only at low speed or with current-limited PSU. */

    /* P1-08 fix: set OCxM to forced inactive BEFORE writing CCER.
     * The original order (CCER first, then CCMR) created a brief
     * window where low-side PWM could occur if the old OCxM was
     * still PWM mode while CCxNE was already set.  Setting forced
     * inactive first guarantees OCxREF=0 before any output is
     * enabled. */
    set_oc_mode_force_inactive(0);
    set_oc_mode_force_inactive(1);
    set_oc_mode_force_inactive(2);

    /* Disable all high-side, enable all low-side */
    uint32_t ccer = TIM1->CCER;
    ccer &= ~(TIM_CCER_CC1E | TIM_CCER_CC2E | TIM_CCER_CC3E);
    ccer |=  (TIM_CCER_CC1NE | TIM_CCER_CC2NE | TIM_CCER_CC3NE);
    TIM1->CCER = ccer;
    ...
    /* Ensure MOE is set (required for output) */
    TIM1->BDTR |= TIM_BDTR_MOE;
...
void MotorDriver_ApplyStep(uint8_t sector, int8_t direction, uint8_t duty)
{
    ...
    /* Safety lock: block gate re-enable after a fault.  Only
     * MotorDriver_SetSafetyLock(false) (called from clrerr path)
     * can unlock. */
    if (s_safety_locked) {
        MotorDriver_AllOff();
        return;
