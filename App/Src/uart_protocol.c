 ...
 * commands that bypass the queue. */
static bool is_emergency_command(const char *line)
{
    if (line == NULL || line[0] == '\0') return false;
    /* Skip leading whitespace */
    while (*line == ' ' || *line == '\t') line++;
    /* Exact matches (case-insensitive handled by caller) */
    if (strcmp(line, "stop") == 0 || strcmp(line, "s") == 0) return true;
    if (strcmp(line, "estop") == 0 || strcmp(line, "safe") == 0) return true;
    if (strcmp(line, "alloff") == 0) return true;
    if (strcmp(line, "rpm 0") == 0) return true;
    if (strcmp(line, "pwm 0") == 0) return true;
    if (strcmp(line, "duty 0") == 0) return true;
    if (strcmp(line, "x") == 0 || strcmp(line, "brake") == 0) return true;
    return false;
}
