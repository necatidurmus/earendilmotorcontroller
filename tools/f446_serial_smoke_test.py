...
def main():
    ...
    with ser:
        ...
        report("m1 status -> response", has_status,
               "F411 may not be connected" if not has_status else "")

        print("\n--- Test 5: m1 hall (requires F411 connected) ---")
        send(ser, "m1 hall")
        lines = read_lines(ser, 1.5)
        has_hall = has_response(lines, "Hall") or has_response(lines, "M1|")
        report("m1 hall -> response", has_hall,
               "F411 may not be connected" if not has_hall else "")

        print("\n--- Test 6: stop (safe stop) ---")
        send(ser, "stop")
        lines = read_lines(ser, 1.0)
        report("stop -> OK", has_response(lines, "OK|safe stop"))
