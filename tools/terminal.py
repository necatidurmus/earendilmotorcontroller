...
class MotorControlGUI(tk.Tk):
    ...
    def _build_movement_panel(self, parent, col):
        ...
        ttk.Spinbox(self.pwm_row, from_=0, to=250, increment=1, textvariable=self.pwm_var, width=6).grid(row=0, column=1, padx=4)
    ...
    def _adjust_value(self, delta: int):
        if self.drive_mode_var.get() == "rpm":
            ...
            new_v = max(0, min(250, self._get_pwm() + delta))
