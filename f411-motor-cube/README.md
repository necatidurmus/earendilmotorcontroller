...
* **No active brake.** `brake` / `x` and the `BRAKE` telemetry field
  are placeholders that **coast** (all gates off). Active brake is
  disabled because there is no current sense and an unverified power
  stage. The `[OK] Brake (coast in first bring-up)` reply makes this
  explicit.
...
| `s` / `stop`             | coast                                                 |
| `x` / `brake`            | **coast** (active brake disabled in first bring-up)   |
...
| `map` / `mapreset`       | show / reset the Hall map                             |
