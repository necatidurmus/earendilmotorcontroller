...
static uint8_t calc_cfg_checksum(uint16_t kickDuty, uint16_t kickMs,
                                  uint16_t rampStep, uint16_t rampIntervalMs,
                                  uint16_t defaultPwm, uint16_t brakeHoldMs)
{
    uint8_t x = 0x33;
    x ^= (uint8_t)(kickDuty & 0xFF);
    x ^= (uint8_t)(kickDuty >> 8);
    x ^= (uint8_t)(kickMs & 0xFF);
    x ^= (uint8_t)(kickMs >> 8);
    x ^= (uint8_t)(rampStep & 0xFF);
    x ^= (uint8_t)(rampStep >> 8);
    x ^= (uint8_t)(rampIntervalMs & 0xFF);
    x ^= (uint8_t)(rampIntervalMs >> 8);
    x ^= (uint8_t)(defaultPwm & 0xFF);
    x ^= (uint8_t)(defaultPwm >> 8);
...
{
    /* raw 0 (0b000) and raw 7 (0b111) are invalid Hall codes and
     * must always be 255 (invalid).  This matches the main validator
     * in Commutation_ValidateHallMap(). */
                        ...
                        uint16_t *defaultPwm, uint16_t *brakeHoldMs)
{
    ...
    uint32_t off = 5;
    uint16_t kd;  memcpy(&kd, base + off, 2); off += 2;
    uint16_t km;  memcpy(&km, base + off, 2); off += 2;
    uint16_t rs;  memcpy(&rs, base + off, 2); off += 2;
    uint16_t ri;  memcpy(&ri, base + off, 2); off += 2;
    uint16_t dp;  memcpy(&dp, base + off, 2); off += 2;
