...
void Telemetry_Tick(uint32_t nowMs)
{
    ...
    if (s_mode == TELEMETRY_DEBUG) {
        ...
        float err     = trmp - frpm;
        uint16_t setd = App_GetTargetDuty();
        uint16_t actd = MotorDriver_GetDuty();
        ...
        UartProtocol_Printf("RPM:%lu,T:%lu,D:%u,DIR:%c,APP_PH:%u,SP:%u,BRAKE:%u,FC:%u,H:%u,PWM_SET:%u,PWM_ACT:%u,QDROP:%lu,RXB:%lu",
                            (unsigned long)rpm,
